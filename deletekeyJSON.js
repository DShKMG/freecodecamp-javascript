...
function delUnwanted() {
    let keyWords = ['oranges', 'plums', 'strawberries'];
    for (const x in foods) {
        //console.log(x);
        if (x == keyWords[0] || x == keyWords[1] || x == keyWords[2]) {
            console.log("Pattern Matched\t Deleting object \"" + x + "\"");
            delete foods[x];
        }
    }
}
